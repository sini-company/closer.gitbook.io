...
여기서 눈썰미가 좋은 사용자라면 두 가지 특별한 플로우가 존재한다는 것을 알아차리셨을 겁니다. 찾으셨나요? 그 두 가지 플로우들은 **시스템 플로우**라고 부르는 플로들우인데, 이 플로우들은 챗봇 동작 과정에서 중요한 역할을 담당합니다.
...
아직 잘 모르시겠다면, 기능을 이해하기 위해 시작 플로우와 폴백 플로우를 변경해봅시다.
...
여기까지 잘 따라오셨다면 CLOSER 챗봇이 기본적으로 어떻게 동작하는지 알게 되셨을 것 같아요. 이제 튜토리얼 챗봇의 내용을 토대로 고객님께서 원하는 동작을 수행하도록 수정해보세요!

## 더 알아보기

CLOSER 챗봇은 기능 수행 단위인 노드들을 유기적으로 배치하고 연결함으로서 동작하게 됩니다.   
각각의 노드들이 어떠한 역할을 수행하는지 알아보고, 원하는 동작을 구현하기 위해서 어떤 노드를 사용하고 구성하여야 하는지 다음 문서들을 통해 더 자세히 알아보세요.
