...
CLOSER에서는 최대한 많은 메시징 채널의 호환성을 위하여, 공통적으로 사용할 수 있는 메시지 형식으로 다음 네 가지 형식을 제공하고 있습니다.
...
단, 지나치게 높은 용량의 파일은 업로드 전 압축되어 화질 손상이 발생할 수 있는 점 유의해주세요.
...
* `cards`: 카드의 배열 \(최대 10개\) 
  ...
  * `buttons`: 카드에 첨부될 버튼 \(최대 9개\), [텍스트 응답 방식](response.md#undefined)에서 사용되는 데이터와 동일
...
* 클릭 시 이동하는 **URL 링크**  \(메신저별로 uri protocol의 지원 여부가 다를 수 있습니다.\)
* \*\*\*\*[**포스트백 페이로드 \(Postback payload\)**](response.md#postback-payload) 

메시지 버튼의 유형은 버튼 선택 시 노출되는 팝업 메뉴를 통해 이용할 수 있습니다. 
