...
포스트백\(Postback\)이란 시스템\(챗봇\)이 전달한 데이터를 사용자가 다시 반환한다는 의미를 가진 용어입니다.   
CLOSER에서 설정는 [**메시지 버튼**](../node/response.md#undefined-2)은 기본적으로 페이로드\(Payload, 전송되는 데이터\) 없는 포스트백 버튼으로, 사용자로 하여금 버튼에 설정된 내용을 그대로 텍스트 메시지로 전송하도록 동작합니다.
...
위와 같은 상황에서 **메시지를 포함한 조건**으로만 챗봇 시나리오를 작성할 경우, 해당 노드의 위치에서 벗어났을 때 챗봇이 적절한 응답을 반환하기 어려워집니다. 이 때, [폴백 플로우](../flow.md#undefined) 에서 어떤 버튼을 눌렀는지 알 수 있도록 포스트백 반환값\(postback payload\)을 활용할 수 있습니다. 
