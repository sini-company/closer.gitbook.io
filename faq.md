...
{% hint style="info" %}
빈 메시지는 [봇 설정 &gt; 시스템 응답 설정 메뉴](builder/chatbot/bot-settings.md#undefined-1)에서 변경하실 수 있습니다.
{% endhint %}

### Q. 오류 메시지를 수정할 수 있나요? <a id="q-error-message"></a>

오류 메시지는 시나리오가 잘못되었거나 노드에 필요한 데이터가 부족한 상태에서 실행될 때 발생합니다. 이 메시지 또한 [봇 정보 설정 &gt; 시스템 응답 설정](builder/chatbot/bot-settings.md#undefined-1) 메뉴에서 수정하실 수 있습니다.
...
상담톡은 PLUS 요금제 이상에서 사용할 수 있으며, 일일 활성 채팅방당 100원의 비용이 발생합니다. \(과금 기준은 대부분의 딜러사가 동일합니다.\) 
...
무료 사용량\(월 1천명\)이 넘어갈 경우에도 챗봇이 중지되지는 않습니다. 대신 메시지 하단에 브랜드 홍보 문구\(powered by closer.ai\)가 포함되어 전송됩니다. 1천명 이하에서는 브랜드 홍보 문구가 표시되지 않습니다.
